...
**Word count.** 6,440 excluding abstract, tables, figure captions and references.
...
**Goodness of fit.** Table 3 compares observed and expected destination distributions by starting state, age group and interval length, excluding censored rows, in the form of Aguirre-Hernández and Farewell (2002). The fit is close from H (largest gap 0.3 points), C (0.8) and D (3.1). It is poor from the two long-term-care states: from L the model expects 47.0% to remain against 39.8% observed and 20.5% to die against 25.8%; from N it expects 56.5% to remain against 41.2% and 36.6% to die against 50.6%, a gap of 15.2 points. The reason is duration: mortality in a nursing home is highest soon after entry, and a Markov model in age alone cannot represent that. The consequence for this paper is that years in N are overstated and deaths from N understated, so lifetime nursing-home cost is, if anything, overstated and lifetime length in that state too long. Section 6.8 reports what a duration term does to the fit.
